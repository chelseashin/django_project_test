...
# 1. settings test
class SettingsTest(TestCase):    # 상속
...
class BoardViewTest(TestCase):
    ...
    # create test에서의 포인트는 'form을 제대로 넘겨주느냐?' 가장 기본은 get_check_200
    def test_01_get_create(self):
        # given when
        with self.login(username='test', password='qawsedrf!'):
            response = self.get_check_200('boards:create')    # get방식으로 create
            # then
    ...
    def test_03_post_create(self):
        ...
        # when 로그인을 해서 post 요청으로 해당url로 요청 보낸 경우
        with self.login(username='test', password='qawsedrf!'):
